...
def FK(angle, link):
    base = TF()
    T0 = TF('y', q = angle[0], dx = 0.000034, dy = -link[0])
    T0_0 = base.dot(T0)
    T1 = TF('x', q = angle[1],dx=0.02506, dy = -link[1], dz = -0.026717)
    T1_0 = T0_0.dot(T1)
    
    T2 = TF('y', q = angle[2],dx = -0.0079, dz = -link[2], dy = -0.0245)
    T2_1 = T1_0.dot(T2)
    
    T3 = TF('z', q = angle[3], dx =-0.0155, dz = -link[3], dy = 0.0245)
    T3_2 = T2_1.dot(T3)

    grip = TF(dx = 0.001081, dz = -0.228470)
    grip_2 = T3_2.dot(grip)
    return base, T0_0, T1_0, T2_1, grip_2
